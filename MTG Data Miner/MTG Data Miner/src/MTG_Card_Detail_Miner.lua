...
function refineCardText(text)
	...
	local capture, pos = "", 1
	repeat
		capture, pos = getDivTagContents(text, pos)
		if capture then
			table.insert(textContents, trim(capture))
		end
	until capture == nil
...
--TODO: Confirm contents will always be inside a div tag
function refineCardFlavor(flavor)
	flavor = string.gsub(flavor, "<i>", "")
	flavor = string.gsub(flavor, "</i>", "")
	return refineCardText(text)
end

function refineCardPt(pt)
	pt = string.gsub(pt, "%s", "")
	values = {}
	string.gsub(pt, "([^/]+)", function(c) table.insert(values, c) end)
	return values
end

--TODO: Capture set abbreviation from img tag as well
function refineCardSet(set)
	string.gsub(set, "title=\"([^\"]+)\"", function(capture) 
		set = capture
	end)
...
function refineCardRarity(rarity)
	return trim(rarity)
end

--TODO: Capture set abbreviation from img tag as well
function refineCardOtherSets(otherSets)
	local sets = {}
	string.gsub(set, "title=\"([^\"]+)\"", function(capture)
		table.insert(sets, capture)
	end)
end
